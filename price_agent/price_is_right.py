            ...
            def get_plot():
                try:
                    ...
                    fig = go.Figure(
                        data=[
                            go.Scatter3d(
                                x=vectors[:, 0] if len(vectors) else [],
                                y=vectors[:, 1] if len(vectors) else [],
                                z=vectors[:, 2] if len(vectors) else [],
                                mode="markers",
                    ...
                    logging.info(f"Plot generation failed (continuing): {e}")
                    fig = go.Figure()
                    fig.update_layout(title="Vector plot unavailable", height=400)
